...
export function Dashboard({ modules: mods, completedLessonIds, getModuleProgress, onNavigate, totalXp, streak, loading, solvedChallenges }: DashboardProps) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8">
      ...
      {/* Playground Section */}
      <div className="mb-6 bg-gradient-to-r from-surface-900 to-surface-900/50 border border-success-500/20 rounded-xl p-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-surface-50 mb-1 flex items-center gap-2">
              <svg className="w-6 h-6 text-success-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 9l3 3-3 3m5 0h3M5 20h14a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
              Playground
            </h2>
            <p className="text-surface-400 text-sm">Free-form ROS2 sandbox. Type commands, write Python, run turtlesim, and experiment freely.</p>
          </div>
          <button
            onClick={() => onNavigate('playground')}
            className="px-5 py-2.5 bg-success-600 hover:bg-success-500 text-surface-50 rounded-lg font-medium transition-colors text-sm"
          >
            Open Playground
          </button>
        </div>
